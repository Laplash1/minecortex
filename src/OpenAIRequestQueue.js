...
class OpenAIRequestQueue {
  constructor(options = {}) {
    ...
    console.log(`[OpenAIRequestQueue] API制御システム初期化: 同時${this.maxConcurrentRequests}件, ${this.requestsPerMinute}req/min`);
  }
  ...
  async enqueueRequest(requestType, requestData, botId = 'unknown', priority = 5) {
    return new Promise((resolve, reject) => {
      ...
      if (this.stats.totalRequests % 10 === 0) {
        console.log(`[OpenAIRequestQueue] キュー状況: ${this.requestQueue.length}待機, ${this.activeRequests.size}実行中`);
      }
    });
  }
  ...
  async executeRequest(request) {
    ...
    try {
      ...
      if (request.retryCount < request.maxRetries && this.shouldRetry(error)) {
        request.retryCount++;
        console.log(`[OpenAIRequestQueue] リトライ ${request.retryCount}/${request.maxRetries}: ${request.id}`);
  ...
  cancelBotRequests(botId) {
    ...
    console.log(`[OpenAIRequestQueue] ボット ${botId} のリクエスト ${actualCancelled}件をキャンセル`);
  }
  ...
  clearQueue() {
    ...
    console.log(`[OpenAIRequestQueue] キューをクリア: ${cancelledCount}件のリクエストをキャンセル`);
  }
   ...
   */
  async shutdown() {
    console.log('[OpenAIRequestQueue] システム終了中...');
    ...
    while (this.activeRequests.size > 0 && (Date.now() - startTime) < maxWait) {
      console.log(`[OpenAIRequestQueue] アクティブリクエスト ${this.activeRequests.size}件の完了待機中...`);
    ...
    console.log(`[OpenAIRequestQueue] 終了時統計: 成功率 ${this.getStats().successRate}%, 完了 ${this.stats.completedRequests}件`);
    console.log('[OpenAIRequestQueue] システム終了完了');
  }
}
